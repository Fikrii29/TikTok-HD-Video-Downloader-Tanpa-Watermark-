...
console.log('[yt-dlp binary]', YOUTUBE_DL_PATH);
...
app.get('/api/youtube/info', async (req, res) => {
  ...
  try {
    const meta = await ytDlp(videoUrl, { dumpJson: true, noPlaylist: true });
...
app.get('/api/youtube/download', (req, res) => {
  ...
  if (fmt === 'mp3') {
    ytArgs      = ['-x', '--audio-format', 'mp3', '--audio-quality', '0', '-o', '-', '--no-playlist', videoUrl];
    ...
    ytArgs      = ['-f', fmtStr, '--merge-output-format', 'mp4', '-o', '-', '--no-playlist', videoUrl];
    filename    = `youtube_video_${Date.now()}.mp4`;
    contentType = 'video/mp4';
  }

  console.log('[YT DL]', YOUTUBE_DL_PATH, ytArgs.slice(-1)[0]);
